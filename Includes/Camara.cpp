...
{
	// Position
	DirectX::XMVECTOR Eye = DirectX::XMVectorSet(0.f, 3.f, -175.0f, 1.f);
	// Direction 
	DirectX::XMVECTOR Up = DirectX::XMVectorSet(0.f, 1.f, 0.f, 1.f);
	// Direction 
	DirectX::XMVECTOR At = DirectX::XMVectorSet(0.f, 1.f, 0.f, 0.f);

	m_View = DirectX::XMMatrixLookAtLH(Eye, At, Up);

	m_View = DirectX::XMMatrixTranspose(m_View);

	m_Projection = DirectX::XMMatrixPerspectiveFovLH(DirectX::XM_PIDIV4, DEFAULT_HEIGHT / (FLOAT)DEFAULT_WIDTH, 0.01f, 500.0f);
